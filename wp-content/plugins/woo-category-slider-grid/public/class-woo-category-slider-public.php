...
class Woo_Category_Slider_Public {
	...
	/**
	 * Register the stylesheets for the live preview.
	 *
	 * @since    1.3.0
	 */
	public function admin_enqueue_styles() {
		...
		// CSS Files.
		if ( $wcsp_options['wcsp_swiper_css'] ) {
			wp_enqueue_style( 'sp-wcs-swiper', SP_WCS_URL . 'public/css/swiper' . $this->suffix . '.css', array(), $this->version, 'all' );
		}
		if ( $wcsp_options['wcsp_fa_css'] ) {
			wp_enqueue_style( 'sp-wcs-font-awesome', SP_WCS_URL . 'public/css/font-awesome.min.css', array(), $this->version, 'all' );
		}
		wp_enqueue_style( $this->plugin_name, SP_WCS_URL . 'public/css/woo-category-slider-public' . $this->suffix . '.css', array(), $this->version, 'all' );

		// Js File.
		wp_enqueue_script( 'sp-wcs-swiper-js', SP_WCS_URL . 'public/js/swiper' . $this->suffix . '.js', array( 'jquery' ), $this->version, true );
