...
if ( ! class_exists( 'SP_WCS_Customize_Options' ) ) {
	/**
	 *
	 ...
	 * @version 1.0.0
	 */
	class SP_WCS_Customize_Options extends SP_WCS_Abstract {
		/**
		 * Unique ID/Name
		 *
		 * @var string
		...
		public function add_customize_save( $wp_customize ) {
			do_action( "spf_{$this->unique}_save_before", $this->get_options(), $this, $wp_customize );
			do_action( "spf_{$this->unique}_saved", $this->get_options(), $this, $wp_customize );
			do_action( "spf_{$this->unique}_save_after", $this->get_options(), $this, $wp_customize );
		}
		...
		public function add_customize_options( $wp_customize ) {
			...
			if ( ! class_exists( 'WP_Customize_Panel_SP_WCS' ) ) {
				SP_WCS::include_plugin_file( 'functions/customize.php' );
			}
		...
		public function add_section( $wp_customize, $section_id, $section_args, $panel_id ) {
			...
			$wp_customize->add_section(
				new WP_Customize_Section_SP_WCS(
					$wp_customize,
					$section_id,
					array(
			...
			if ( ! empty( $section_args['fields'] ) ) {
				...
				foreach ( $section_args['fields'] as $field ) {
					...
					$wp_customize->add_control(
						new WP_Customize_Control_SP_WCS(
							$wp_customize,
							$setting_id,
							wp_parse_args(
								$control_args,
