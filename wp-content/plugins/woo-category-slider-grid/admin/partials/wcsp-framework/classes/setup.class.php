	...
	class SP_WCS {
			...
			if ( ! empty( self::$args['customize_options'] ) ) {
				foreach ( self::$args['customize_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						SP_WCS_Customize_Options::instance( $key, $params );
			...
			if ( ! empty( self::$args['profile_options'] ) ) {
				foreach ( self::$args['profile_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						$params['sections']   = self::$args['sections'][ $key ];
						self::$inited[ $key ] = true;
						SP_WCS_Profile_Options::instance( $key, $params );
					}
				}
			}
			...
			if ( ! empty( self::$args['shortcoders'] ) ) {
				...
				foreach ( self::$args['shortcoders'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						SP_WCS_Shortcoder::instance( $key, $params );
				...
				if ( ! empty( self::$shortcode_instances ) ) {
					SP_WCS_Shortcoder::once_editor_setup();
				}
			}
			...
			if ( ! empty( self::$args['taxonomy_options'] ) ) {
				foreach ( self::$args['taxonomy_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						SP_WCS_Taxonomy_Options::instance( $key, $params );
			...
			if ( ! empty( self::$args['widgets'] ) && class_exists( 'WP_Widget_Factory' ) ) {
				...
				foreach ( self::$args['widgets'] as $key => $value ) {
					if ( ! isset( self::$inited[ $key ] ) ) {
						self::$inited[ $key ] = true;
						$wp_widget_factory->register( SP_WCS_Widget::instance( $key, $value ) );
					}
				}
			}
			...
			if ( 'sp_wcslider' === $the_current_post_type || 'product_cat' === $the_current_taxonomy ) {
				...
				wp_localize_script(
					...
					array(
						...
						'i18n'          => array(
							'confirm'             => esc_html__( 'Are you sure?', 'woo-category-slider' ),
							'reset_notification'  => esc_html__( 'Restoring options.', 'woo-category-slider' ),
							'import_notification' => esc_html__( 'Importing options.', 'woo-category-slider' ),
						),
					)
				);
		...
		public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
			...
			if ( ! empty( $field['_notice'] ) ) {
				...
				$field['content'] = sprintf( esc_html__( 'Ooops! This field type (%s) can not be used here, yet.', 'woo-category-slider' ), '<strong>' . $field_type . '</strong>' );
				$field['type']    = 'notice';
				$field['style']   = 'danger';
			...
			if ( ! empty( $field_type ) ) {
				...
				if ( class_exists( $classname ) ) {
					...
					echo '<p>' . esc_html__( 'This field class is not available!', 'woo-category-slider' ) . '</p>';
				}
			} else {
				echo '<p>' . esc_html__( 'This type is not found!', 'woo-category-slider' ) . '</p>';
			}
