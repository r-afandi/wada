	...
	class SP_WCS_Metabox extends SP_WCS_Abstract {
		...
		public function add_meta_box_content( $post, $callback ) {
			...
			if ( ! empty( $this->args['show_restore'] ) ) {
				...
				echo '<span class="button spf-button-restore">' . esc_html__( 'Restore', 'woo-category-slider' ) . '</span>';
				echo '<span class="button spf-button-cancel">' . sprintf( '<small>( %s )</small> %s', esc_html__( 'update post for restore ', 'woo-category-slider' ), esc_html__( 'Cancel', 'woo-category-slider' ) ) . '</span>';
				echo '</label>';
				echo '</div>';
