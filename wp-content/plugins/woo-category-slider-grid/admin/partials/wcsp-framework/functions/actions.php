 ...
 * @return void
 */
function wpsp_wcs_get_taxonomies() {
	extract( $_REQUEST );
	$taxonomy_names = get_object_taxonomies( array( 'post_type' => $wpsp_post_type ), 'names' );
	foreach ( $taxonomy_names as $key => $label ) {
		echo '<option value="' . $label . '">' . $label . '</option>';
	}
	die( 0 );
}
add_action( 'wp_ajax_wpsp_wcs_get_taxonomies', 'wpsp_wcs_get_taxonomies' );
 ...
 * @return void
 */
function wpsp_wcs_get_terms() {
	extract( $_REQUEST );
	$terms = get_terms( $wpsp_post_taxonomy );
	foreach ( $terms as $key => $value ) {
		echo '<option value="' . $value->term_id . '">' . $value->name . '</option>';
	}
	die( 0 );
}
add_action( 'wp_ajax_wpsp_wcs_get_terms', 'wpsp_wcs_get_terms' );
 ...
 * @return void
 */
function wpsp_wcs_get_posts() {
	extract( $_REQUEST );
	$all_posts = get_posts(
		array(
			'post_type'      => $wpsp_post_type,
			'posts_per_page' => -1,
		)
	);
...
add_action( 'wp_ajax_wpsp_wcs_get_posts', 'wpsp_wcs_get_posts' );
	...
	function spf_get_icons() {
		...
		if ( ! empty( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'spf_icon_nonce' ) ) {
			...
			if ( ! empty( $icon_lists ) ) {
				...
				foreach ( $icon_lists as $list ) {
					...
					foreach ( $list['icons'] as $icon ) {
						...
						echo '<div class="spf-text-error">' . esc_html__( 'No data provided by developer', 'woo-category-slider' ) . '</div>';
			...
			wp_send_json_error(
				array(
					'success' => false,
					'error'   => esc_html__( 'Error while saving.', 'woo-category-slider' ),
					'debug'   => $_REQUEST,
				)
			);
...
if ( ! function_exists( 'spf_export' ) ) {
	...
	function spf_export() {
		...
		if ( ! empty( $_GET['export'] ) && ! empty( $_GET['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['nonce'] ) ), 'spf_backup_nonce' ) ) {
			...
			echo wp_json_encode( get_option( wp_unslash( $_GET['export'] ) ) );
...
if ( ! function_exists( 'spf_import_ajax' ) ) {
	...
	function spf_import_ajax() {
		...
		if ( ! empty( $_POST['import_data'] ) && ! empty( $_POST['unique'] ) && ! empty( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'spf_backup_nonce' ) ) {
			...
			<div id="spf-modal-icon" class="spf-modal spf-modal-icon">
				...
				<div class="spf-modal-table-cell">
					...
					<div class="spf-modal-title">
						<?php esc_html_e( 'Add Icon', 'woo-category-slider' ); ?>
						<div class="spf-modal-close spf-icon-close"></div>
					</div>
					<div class="spf-modal-header spf-text-center">
						<input type="text" placeholder="<?php esc_html_e( 'Search a Icon...', 'woo-category-slider' ); ?>" class="spf-icon-search" />
					</div>
