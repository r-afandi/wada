	...
	class SP_WCS_Field_image_sizes extends SP_WCS_Fields {
		...
		public function render() {
			...
			$sizes = array_merge(
				$sizes,
				array(
					'full'   => __( 'Original uploaded image', 'woo-category-slider' ),
					'custom' => __( 'Set custom size', 'woo-category-slider' ),
				)
			);
			...
			if ( ! empty( $sizes ) ) {
				...
				if ( ! empty( $sizes ) ) {
					...
					echo ! empty( $this->field['empty_message'] ) ? esc_html( $this->field['empty_message'] ) : esc_html__( 'No image sizes found.', 'woo-category-slider' );
