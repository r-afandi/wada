<?php
/**
 * Framework checkboxf field file.
 *
			...
			if ( ! empty( $this->field['options'] ) ) {
				...
				if ( ! empty( $options ) ) {
					...
					foreach ( $options as $option_key => $option_value ) {
						$pro_only = $option_value['pro_only'] ? 'disabled' : '';
						$checked  = ( in_array( $option_key, $value, true ) ) ? ' checked' : '';
						echo '<li><label class="wcs-' . esc_attr( $pro_only ) . '"><input ' . esc_attr( $pro_only ) . ' type="checkbox" name="' . esc_attr( $this->field_name( '[]' ) ) . '" value="' . esc_attr( $option_key ) . '"' . wp_kses_post( $this->field_attributes() . $checked ) . '/> ' . esc_html( $option_value['text'] ) . '</label></li>';
					}
					echo '</ul>';

				} else {

					echo ! empty( $this->field['empty_message'] ) ? esc_html( $this->field['empty_message'] ) : esc_html__( 'No data provided for this option type.', 'woo-category-slider' );
