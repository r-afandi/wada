			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( ! empty( $options ) ) {
					...
					foreach ( $options as $option_key => $option_value ) {
						...
						echo '<li><label class="wcs-' . esc_attr( $pro_only ) . '"><input ' . esc_attr( $pro_only ) . ' type="radio" name="' . esc_attr( $this->field_name() ) . '" value="' . esc_attr( $option_key ) . '"' . wp_kses_post( $this->field_attributes() . $checked ) . '/> ' . esc_html( $option_value ) . '</label></li>';
					}
					echo '</ul>';
